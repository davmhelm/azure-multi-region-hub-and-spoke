# Lab - Azure Virtual Network Scenario: Hub and Spoke networking in multiple regions with Azure Firewalls for inter-hub routing

## Intro

The goal of this lab is to demonstrate and validate traffic flow between spoke VNets in different regions. Spoke VNets are peered to a hub VNet in each region. Hub VNets are peered to each other through global VNet peering. It is a sample implementation of [Use Azure Firewall to route a multi hub and spoke topology | Azure Firewall documentation](https://learn.microsoft.com/en-us/azure/firewall/firewall-multi-hub-spoke), which I have guided customers through successfully many times in the past.

### Lab Diagram

Below is a diagram of what you should expect to be deployed:

![net diagram](./LabEnvironment.png)

### Components

- Two "Sites", Site1 and Site2, corresponding to an Azure region (Default West Central US and Central US, respectively)
- Two hub VNets, one for each "Site"
    - The hub VNets each have an Azure Firewall Standard SKU deployed, with a single public IP address
    - The Azure Firewalls are configured to send logs & metrics to a Log Analytics workspace in Site1
    - Network rules are configured on the Azure Firewall to permit traffic sourced from RFC1918 address space, destined anywhere
- One spoke VNet per site
    - A Linux VM (Ubuntu) is deployed in each spoke VNet
    - Custom storage accounts are deployed for each site and VMs are configured to use them for boot diagnostics -- so you can use Azure Serial Console for remote management (no public IPs or Azure Bastion required)
- Network Security Groups
    - Applied for illustrative purposes to the Spoke VNets' VmSubnet
    - Inbound and Outbound Rules are left at defaults
- Route tables
    - Applied to the Hub VNets in each site, on the AzureFirewallSubnet
        - The opposite site's summary address space is specified to use the Azure Firewall in that site as the next hop
        - Default route to the Internet explicitly required for route tables applied to AzureFirewallSubnet
    - Applied to the Spoke VNets in each site, on the VmSubnet
        - BGP route propagation is disabled to avoid the RFC1918 drop routes that normally get programmed onto route tables
        - A single default route is set to use the private IP address of the Azure Firewall in that site as the next hop

Upon complete deployment of the environment, Azure Network Watcher tests are conducted to validate communication between the VMs in the spokes, in each direction (site1->site2 and site2->site1)
...
cd ./azure-multi-region-hub-and-spoke
...
echo -e "\taz group delete --name $rgname \\ \n\t  --force-deletion-types Microsoft.Compute/virtualMachines --yes"
echo -e "\n"
